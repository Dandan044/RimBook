...
| 重写污染 | 预写快照 + 干净回退（`delete_missing`）/ 后续章存在时分支 fork |
| 问题溯源 | LLM Trace：每次调用记录到项目 `.llm_logs/`（按日 JSONL） |
...
┌──────────────────────────────────────────────────────────────────────┐
│                         Web UI (Vue 3)                                │
│  仪表盘 │ 设定集 │ 大纲 │ 写作工作台 │ 叙事管理 │ 工作流 │ 设置   │
│                                                                        │
│  · 设定集：Tab 式（档案 / 章节发现 / 矛盾）                           │
│  · 叙事管理：线索账本 / 风格指南 / 分层记忆 / 宏观审阅               │
...
│ 发现/矛盾 │ 风格指南 │ 线索账本  │ Versioning（快照/分支/回退）      │
│          │ 全书至今 │ 卷回顾    │ Trace（.llm_logs 溯源）            │
├──────────┴──────────┴───────────┴────────────────────────────────────┤
...
├── versioning/         # 快照 / 分支 / 干净回退（delete_missing）
├── retrieval/          # 可选向量检索（ChromaDB）
└── web/                # Web 层：FastAPI 后端 + 静态前端
    ├── backend/        # API 路由（含 narrative）、SSE、依赖注入
...
| **大纲** | 树状浏览梗概→卷→章节，LLM 生成或手动编辑 beat，章节草稿状态 |
| **写作工作台** | 三栏布局：结构化上下文 / 正文编辑器 / 校验与富化；支持当前分支显示、重写回退与后续章存在时的分支 fork |
| **叙事管理** | 线索账本、风格指南、全书至今 / 卷回顾、宏观编辑审阅 |
...
| 路径 | 条件 | 行为 |
|------|------|------|
| 简单回退 | 该章有草稿，后续章无草稿 | 从最早的 `write-chN-` 预写快照恢复，并删除快照中不存在的 codex/state 文件 |
