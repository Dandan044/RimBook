...
│                                                                        │
│  · 写作规划：卷规划 v2（写作框架 → 详尽大纲 → 设定扩充 → beat 链 → 章基调+细场景）/ 单章补规划 / 删除卷章 │
...
1. **Step 1** — **写作框架与详述出场**：全量设定进上下文；产出读者透镜、手法配比、舞台与角色详述（非剧情大纲），落盘 `volNN.framework.yaml`
2. **Step 2** — **详尽卷大纲**：仅注入涉及实体的全量设定 + 框架，生成 title / 加长 arc / ending / chapter_count
3. **Step 3** — 本卷设定扩充（可新建/更新设定，并参考框架避免功能重叠）
4. **Step 4** — 生成整卷连续叙事 beat 链（不按章切分；有数量上下限）
5. **Step 5a** — 按节奏分组为章，并为每章写 **章基调**（`keynote`：隐性约束，渗透正文但不明说）
6. **Step 5b** — 按章把每个 beat 拆成 **细场景**（`MicroScene`）

Writer 写正文时优先服从章基调与细场景：基调只渗透不明说；细场景按意图演出，空字段勿补人物戏。  
章基调建议 2–5 条本章特有约束，勿套公式前缀。
全流程 SSE 一次跑完；可编辑 beat 后单独重跑组装步骤（「重新组装」）。中间状态保存在 `outline/volumes/volNN.framework.yaml` 与 `volNN.beats.yaml`。
...
| `outline volume` | 规划一卷（CLI/Web「新卷」走 v2：写作框架 → 详尽大纲 → 设定扩充 → beat 链 → 章基调+细场景） |
...
| **写作规划** | 大纲树 + 卷规划 v2（写作框架/详尽大纲/beat 链五步 SSE）+ 章基调与细场景编辑；线索账本 / 风格指南 / 故事线 / 宏观审阅 |
