...
class Prompts:
    ...
    volume_system: str = (
        ...
        "关键场景与转折、本卷张力曲线设计（避免平淡，应有起伏与高潮）、"
        "本卷应埋下与回收的伏笔规划、以及与下一卷的衔接。控制在 400-600 字。"
    )
    ...
    writer_system: str = (
        ...
        "6. 数值、时间、距离、年龄等描写需前后一致，避免计算错误与数量矛盾；\n"
        "7. 因果关系必须成立，设定规则的运用需自洽"
        "（如时间流速变化的后果应符合设定）；\n"
        "8. 避免陈词滥调与翻译腔，自然衔接前文末尾，保持行文连贯；\n"
        "9. 场景转换时交代清楚角色位置与时间推移，避免空间跳跃混乱；\n"
        "10. 输出纯净的小说正文，不要任何标题、注释或元说明。"
    )

    writer_user: str = (
        "请根据以下结构化上下文，撰写第 {number} 章的完整正文。\n\n"
        "{context}\n\n"
        "现在请直接开始写正文。严格遵循上下文中的章节 beat 与风格指南，"
        "注意数值一致、因果自洽与场景转换交代。"
    )
    ...
    writer_revise_system: str = (
        ...
        "【修订原则】\n"
        "- 即使用户未在「修订要求」中指明，也必须主动发现并修正上述七类问题；\n"
    ...
    checker_system: str = (
        ...
        "设定集、实体档案、前文摘要与本章 beat，找出一切不一致与错误之处。\n"
        "检查维度：\n"
    ...
    checker_user: str = (
        ...
        '      "category": "setting|character|plot|fact|logic|numeric|commonsense",\n'
        '      "description": "问题的具体描述",\n'
        '      "evidence": "正文中的相关原文",\n'
        '      "suggestion": "修复建议"\n'
        '    }}\n'
